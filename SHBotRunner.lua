...
--local CHAR_NAME = "Asamiya";
local CHAR_NAME = "Lrrr-Omicroninan";
local z = 1;
local ticktock = 0;
local ticktack = 0;
local friend = false;
...
function inMap()
	...
	if a ~= SH_MAPID and friend == true then
...
function outMap()
	...
	if a == SH_MAPID then
		...
		--INFO("tick %d",tick);
		if tick > 100 and flag_emergency == 0 and CHAR_ISDEAD == false then
...
function start()
	...
	if a ~= SH_MAPID then
		...
		if IS_NECRO == 1 then
			INFO("BOT RUNNER START MAP NECRO : %s",mode);
		end
