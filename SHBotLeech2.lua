...
function start()
	...
	if a ~= SH_MAPID then
		...
		if IS_BEAST == 1 then
			INFO("BOT LEECH %s MAP BEAST : %s",CHAR_CLASS,mode);
		end
		if IS_NECRO == 1 then
			INFO("BOT LEECH %s MAP NECRO : %s",CHAR_CLASS,mode);
		end
		...
		hell_global.sendCommand("/local mode:megaparty");
		movebot_handle_center = register(event.ONTICK, function() thenMoveToCenter(44.26,-43.08,6.46) end);
...
function startSell(tabledata_buy) 
	register("STARTUP", function(e)
		...
		INFO("Auto Seller - all %s run",RUN_SELL);
		for pattern,quantity in pairs(tabledata_buy) do
